...
TRANSACTION_TABLE = f"{DATABASE}.TRS_v10"
CUSTOMER_TABLE = f"{DATABASE}.CUSTOMERS"
ACCOUNT_TABLE = f"{DATABASE}.ACCOUNTS"
MERCHANT_TABLE = f"{DATABASE}.MERCHANTS"
BRANCH_TABLE = f"{DATABASE}.BRANCHES"
...
spark = (
    ...
    # Do not let AQE collapse all of our intentionally
    # substantial shuffle parallelism.
    .config(
        "spark.sql.adaptive.coalescePartitions.enabled",
        "false"
    )

    # Keep skew handling from changing the workload topology
    ...
    # Important:
    # Force shuffle-based joins rather than broadcast joins.
    .config(
