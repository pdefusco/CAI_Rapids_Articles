...
#### 7. Overview of Spark Rapids Configurations
...
| `spark.executor.resource.gpu.amount = 1`                                                                                                                            | Requests 1 GPU resource per executor. This is how Spark knows to assign a GPU to each executor.                                                                                                                                                                                                                    |
| `spark.executor.instances = 1`                                                                                                                                      | Requests exactly 1 executor. Combined with dynamic allocation disabled, this means a static single executor.                                                                                                                                                                                                       |
| `spark.executor.memory = 16g`                                                                                                                                       | Allocates 16 GB of JVM heap memory per executor for Spark tasks.                                                                                                                                                                                                                                                   |
| `spark.rapids.memory.pinnedPool.size = 2G`                                                                                                                          | For the RAPIDS GPU plugin: pins 2 GB of host memory per executor. Pinned memory helps speed up GPU ↔ host transfers. ([NVIDIA Docs][3])                                                                                                                                                                            |
| `spark.plugins = com.nvidia.spark.SQLPlugin`                                                                                                                        | Enables the NVIDIA RAPIDS Accelerator plugin in Spark, so Spark can offload certain SQL workloads to GPU.                                                                                                                                                                                                          |
| `spark.jars.packages = com.nvidia:rapids-4-spark_2.12:25.08.0`                                                                                                      | Pulls in the RAPIDS Accelerator jar from Maven (or whatever repository), version `25.08.0`, compiled for Scala 2.12.                                                                                                                                                                                               |
| `spark.executor.resource.gpu.discoveryScript = /home/cdsw/getGpusResources.sh`                                                                                      | Points to a script that Spark will run on each executor node to discover how many GPUs are available (and where), so it can assign GPUs correctly.                                                                                                                                                                 |
| `spark.executor.resource.gpu.vendor = nvidia.com`                                                                                                                   | Identifies the GPU vendor; in Kubernetes (or other resource managers) this ties the GPU resource to the correct vendor. Matches how Kubernetes or YARN labels GPU resources.                                                                                                                                       |
| `spark.rapids.shims-provider-override = com.nvidia.spark.rapids.shims.spark330.SparkShimServiceProvider`                                                            | Specifies which “shim” provider implementation to use. Shims help the RAPIDS plugin work with a specific Spark version (“spark330” = Spark 3.3.0).                                                                                                                                                                 |
| `spark.driver.memory = 10g`                                                                                                                                         | Allocates 10 GB memory to the Spark driver (the process that coordinates the job).                                                                                                                                                                                                                                 |
| `spark.eventLog.enabled = true`                                                                                                                                     | Enables event logging, so you can record Spark events (for UI / history server).                                                                                                                                                                                                                                   |
| `spark.rapids.sql.concurrentGpuTasks = 2`                                                                                                                           | Configures how many Spark tasks can run *concurrently on each GPU*. As per NVIDIA tuning guide, limiting concurrency helps avoid out-of-memory on GPU. ([NVIDIA Docs][4])                                                                                                                                          |
| `spark.sql.files.maxPartitionBytes = 256m`                                                                                                                          | Sets the maximum number of bytes to pack into a single partition when reading files via Spark’s DataSource API. Larger partitions help to give each GPU more work per task, improving efficiency. ([NVIDIA Docs][3])                                                                                               |
| `spark.locality.wait = 0`                                                                                                                                           | How long Spark waits to get data-local tasks before scheduling non-local tasks. Setting to zero means “don’t wait” for locality, which may help when locality is not important / when optimizing for throughput. In RAPIDS tuning guide, reducing locality wait can be useful when I/O is fast. ([NVIDIA Docs][4]) |
| `spark.sql.adaptive.enabled = true`                                                                                                                                 | Enables Spark’s Adaptive Query Execution (AQE), which can dynamically optimize query execution (e.g., coalesce partitions, change join strategies) at runtime.                                                                                                                                                     |
| `spark.rapids.memory.pinnedPool.size = 2g`                                                                                                                          | (Duplicate) Again sets pinned host memory pool size; same as earlier.                                                                                                                                                                                                                                              |
| `spark.sql.adaptive.advisoryPartitionSizeInBytes = 1g`                                                                                                              | For AQE: advisory target partition size is 1 GB. This helps Spark decide how to coalesce shuffle partitions. In RAPIDS / Cloudera tuning, this is used to tune partitioning size. ([Cloudera Documentation][5])                                                                                                    |
| `spark.executor.memoryOverhead = 3g`                                                                                                                                | Extra off-heap memory per executor beyond the JVM heap. Needed in GPU setups because of pinned memory and spill. In Cloudera tuning docs, memoryOverhead should account for pinned memory. ([Cloudera Documentation][5])                                                                                           |
| `spark.kryo.registrator = com.nvidia.spark.rapids.GpuKryoRegistrator`                                                                                               | Uses a custom Kryo registrator provided by RAPIDS, so GPU-serialized classes (e.g. GPU columnar batches) are handled efficiently by Kryo.                                                                                                                                                                          |
| `spark.rapids.sql.enabled = true`                                                                                                                                   | Enables RAPIDS SQL acceleration: Spark’s SQL engine will attempt GPU-accelerated operations when possible.                                                                                                                                                                                                         |
| `spark.rapids.sql.incompatibleOps.enabled = true`                                                                                                                   | Enables “incompatible” operations to run on GPU even if they are not fully compatible / optimized; this may fallback or incur overhead but allows more SQL operations to run.                                                                                                                                      |
| `spark.rapids.sql.udfCompiler.enabled = true`                                                                                                                       | Enables the UDF compiler in RAPIDS, which can compile certain user-defined functions to GPU code for acceleration.                                                                                                                                                                                                 |
| `spark.rapids.sql.format.csv.read.enabled = true`                                                                                                                   | Enables GPU acceleration for reading CSV files. According to RAPIDS docs, CSV read on GPU must be explicitly turned on. ([NVIDIA GitHub][6])                                                                                                                                                                       |
| `spark.rapids.sql.format.csv.enabled = true`                                                                                                                        | Enables both read and write (or full) CSV acceleration (or at least more CSV operations) through RAPIDS.                                                                                                                                                                                                           |
| `spark.rapids.sql.variableFloatAgg.enabled = true`                                                                                                                  | Enables RAPIDS-accelerated aggregation for `float` (or variable precision floats) types.                                                                                                                                                                                                                           |
| `spark.rapids.sql.explain = ALL`                                                                                                                                    | Enables verbose explain output from the RAPIDS plugin: Spark will print out detailed GPU plan / operator info in the explain plan, which helps debugging / tuning.                                                                                                                                                 |
| `spark.sql.hive.convertMetastoreParquet = true`                                                                                                                     | When reading Parquet from Hive metastore tables, convert them to Spark-native Parquet rather than using Hive SerDe; this helps Spark SQL performance.                                                                                                                                                              |
| `spark.rapids.sql.castFloatToString.enabled = true`                                                                                                                 | Enables GPU-accelerated casts from float to string. Useful if your queries include such conversions.                                                                                                                                                                                                               |
| `spark.rapids.sql.csv.read.float.enabled = true`                                                                                                                    | Enables GPU acceleration when reading floats from CSVs. This is more specific than the generic CSV read config.                                                                                                                                                                                                    |
| `spark.rapids.sql.castStringToFloat.enabled = true`                                                                                                                 | Enables GPU-accelerated casts from string to float. Useful for CSV data where float columns are parsed from strings.                                                                                                                                                                                               |
| `spark.hadoop.fs.s3a.custom.signers = RazS3SignerPlugin:org.apache.ranger.raz.hook.s3.RazS3SignerPlugin:org.apache.ranger.raz.hook.s3.RazS3SignerPluginInitializer` | This is a Hadoop (S3A) configuration telling Spark to use a custom S3 signer plugin (Ranger RAZ) for S3 authentication / signing. This is probably required in your environment to authorize access via Ranger / RAZ.                                                                                              |
| `spark.hadoop.fs.s3a.s3.signing-algorithm = RazS3SignerPlugin`                                                                                                      | Tells the S3A filesystem implementation to use the “RazS3SignerPlugin” signing algorithm. This works together with the custom signer above.                                                                                                                                                                        |
